...
UPDATE global_var   -- Update per uniformare il nome delle nazioni in ogni tabella
...
UPDATE co2_over_years  -- Update per uniformare il nome delle nazioni in ogni tabella
...
UPDATE food     -- Update per uniformare il nome delle nazioni in ogni tabella
SET area = 'Russia'
...
WHERE 
	country IN(         
				SELECT country_name     -- subquery per usare come condizione WHERE i paesi da mostrare siano i top 5 produttori di co2
				FROM global_var            
...
FROM co2_over_years
WHERE 
	country LIKE 'China'    --mi concentro solo sulla cCina
...
SELECT	product, 
		kgco2_per_kgproduct AS kg_co2_per_1kg_product
FROM food_co2
ORDER BY kg_co2_per_1kg_product DESC
LIMIT 3;
					-- Beef
				    -- Dark chocolate
					-- Lamb & Mutton
...
--- Unendo questo risultato con le info relative alla quantità di co2 prodotta per unità di prodotto (JOIN) ottengo la quantità di co2 prodotta
--- dal singolo paese per la produzione del singolo alimento.
...
--- ! Poiche la fao non ha ancora pubblicato i dati in merito alle produzioni globali per il 2023 le stime seguenti sono state calcolate per l’anno 2022 !
...
AS(
	...
	--%co2 da chocolate
	SELECT	area, 
			'Dark Chocolate' AS item, food_value AS tons_item
	FROM food
	WHERE 
	    area LIKE 'China' 
	    AND item ~* 'cocoa' 
